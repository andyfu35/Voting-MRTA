...
AUCTION_EPSILON = 1e-8
AUCTION_MAX_ITERATIONS = 200_000
...
def validate_experiment_config(
    ...
    if robot_count < 2:
        fail(
            ...
            "contract",
            "ROBOT_COUNT_TOO_SMALL",
            f"actual={robot_count}",
        )
...
def sample_p2p_cost_visibility(
    ...
    Sender->receiver delivery is independent for every task-cost scalar.
    Each receiver always knows its own sender row.
    """
    visible = (
        rng.random((robot_count, robot_count, task_count))
        >= packet_loss_rate
    )
...
def solve_hungarian_assignment(
    ...
    if np.any(assignment < 0):
        return None
    if np.any(
        ~np.isfinite(costs[assignment, np.arange(costs.shape[1])])
    ):
        return None
    return assignment


def solve_sequential_greedy(
    costs: np.ndarray,
    ...
    if set(task_order.tolist()) != set(range(task_count)):
        fail(
            ...
            "contract",
            "INVALID_TASK_ORDER",
            f"task_order={task_order.tolist()}",
        )
    ...
    for task in task_order:
        candidates = np.flatnonzero(
            available & np.isfinite(costs[:, task])
        )
        if len(candidates) == 0:
            return None
        chosen = int(
            candidates[np.argmin(costs[candidates, task])]
        )
...
def solve_batched_auction_assignments(
    receiver_costs: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Run a Bertsekas-style auction for every receiver-local cost matrix.

    The batch dimension is receiver. Missing P2P costs are +inf and therefore
    cannot be bid on. The result is one task->robot proposal per feasible
    receiver. Auction solves the same capacity-one assignment objective as
    Hungarian but through iterative price/bid updates.
    """
    ...
    while True:
        ...
        task_indices = np.argmax(
            unassigned[receiver_indices],
            axis=1,
        )

        task_costs = receiver_costs[
            receiver_indices,
            :,
            task_indices,
        ]
        ...
        iterations += 1
        if iterations > AUCTION_MAX_ITERATIONS:
            fail(
                "solve_batched_auction_assignments",
                "planning",
                "AUCTION_DID_NOT_CONVERGE",
                (
                    f"tasks={task_count} "
                    f"active_receivers={int(active.sum())}"
                ),
            )

    valid = feasible & np.all(assignment >= 0, axis=1)
...
def solve_local_optimizer_proposals(
    ...
    for receiver in range(receiver_count):
        ...
        if method == "p2p_greedy":
            proposal = solve_sequential_greedy(
                local_costs,
                task_order,
            )
    ...
    if support.shape != tie_priority.shape:
        fail(
            ...
            (
                ...
                f"tie_priority={tie_priority.shape}"
            ),
        )
...
def optimizer_consensus_assignment(
    *,
    ...
    receiver_costs = build_receiver_cost_views(
        costs,
        visibility,
    )
    ...
    assignment = solve_support_consensus(
        support,
        tie_priority,
    )
    valid_rate_percent = 100.0 * float(valid.mean())
    return assignment, valid_rate_percent
...
def validate_zero_loss_optimizer_contract(seed: int) -> None:
    ...
    for task_count in (1, 5, 50, 100):
        costs = generate_spatial_cost_matrix(
            ROBOT_COUNT,
            task_count,
            rng,
        )
        ...
        for method in exact_methods:
            ...
            actual_cost = assignment_total_cost(
                costs,
                assignment,
            )
            gap_percent = (
                100.0
                * (actual_cost - oracle_cost)
                / oracle_cost
            )
            if valid_rate != 100.0:
                fail(
...
def evaluate_assignment(
    *,
    ...
    total_cost = assignment_total_cost(
        costs,
        assignment,
    )
    gap_percent = (
    ...
    return {
        ...
        "near_optimal_5pct": (
            gap_percent
            <= NEAR_OPTIMAL_GAP_PERCENT + 1e-12
        ),
...
def run_trial(
    *,
    ...
    optimal_cost = assignment_total_cost(
        costs,
        optimal_assignment,
    )
...
def run_experiment(
    *,
    ...
    print(
        ...
        "(Hungarian/Auction match oracle; "
        "single-task Greedy matches oracle)"
    )
    ...
    for task_count in task_counts:
        ...
        print(
            f"tasks={task_count:3d}/"
            f"{max(task_counts)} complete"
        )

    raw = pd.DataFrame.from_records(records)
    summary = (
        raw.groupby(
            ["tasks", "method", "method_label"],
            as_index=False,
        )
        .agg(
            average_total_cost=(
            ...
            average_valid_proposal_rate_percent=(
                "valid_proposal_rate_percent",
                "mean",
            ),
        )
...
def report_table(
    ...
    return (
        table.reindex(columns=ordered_labels)
        .reset_index()
    )
...
def save_outputs(
    ...
    ensure_output_dirs()
    raw.to_csv(
        DATA_DIR / "optimizer_comparison_raw.csv",
        index=False,
    )
    summary.to_csv(
        DATA_DIR / "optimizer_comparison_summary.csv",
        index=False,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare real multi-task assignment optimizers with "
            "100 robots under paired directed P2P cost loss. "
            "Hungarian is the full-information oracle; P2P "
            "Hungarian and Auction optimize each receiver's "
            "incomplete assignment matrix; P2P Greedy is the "
            "heuristic baseline."
        )
    )
    parser.add_argument(
...
def main() -> None:
    ...
    print(
        "\nValid local optimizer proposals (%) "
        "- diagnostic:"
    )
    print(
        report_table(
